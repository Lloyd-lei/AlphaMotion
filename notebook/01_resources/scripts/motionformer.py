...
class MotionFormer(nn.Module):
    def __init__(self, cfg: MotionFormerConfig):
        ...
        # Stage-1 FM input: per-joint vector = pos(3) + rot6d(6) + root_broadcast(3) = 12 channels.
        # Plus a time-conditioning embedding added to the MSA stream (sinusoidal → MLP).
        IN_CH_FM = cfg.C + 6 + 3
        ...
        # Time conditioning (FM): sinusoidal_t_embed(t) → MLP → broadcast bias on MSA
    ...
    def forward(
        self,
        x_pos_t:  torch.Tensor,        # [B, T, J, 3]   noisy pos at time t
        x_rot_t:  torch.Tensor,        # [B, T, J, 6]   noisy rot6d at time t
        x_root_t: torch.Tensor,        # [B, T, 3]      noisy root at time t
        t:        torch.Tensor,        # [B]            in [0, 1]
        mask:     torch.Tensor | None = None,  # [B, T, J] optional inpainting mask
    ) -> dict:
        """Conditional flow-matching forward.

        Returns velocity-field predictions:
            {"pos": [B,T,J,3], "rot6d": [B,T,J,6], "root": [B,T,3]}
        Each is the predicted velocity v_θ(x_t, t) for its respective stream.
        """
        B, T, J, _ = x_pos_t.shape

        # Concatenate all noisy streams into a per-joint 12-channel input.
        x_root_b = x_root_t.unsqueeze(2).expand(B, T, J, 3)        # [B, T, J, 3]
        x_in     = torch.cat([x_pos_t, x_rot_t, x_root_b], dim=-1) # [B, T, J, 12]

        msa = self.input_proj(x_in)
        msa = msa + self.time_pe[None, :, None, :] + self.joint_pe[None, None, :, :]

        # Time conditioning: sinusoidal embed → MLP → broadcast bias
        t_emb = _sinusoidal_t_embed(t, self.t_embed_dim)            # [B, t_embed_dim]
        t_proj = self.t_mlp(t_emb)                                  # [B, hidden]
        msa = msa + t_proj.view(B, 1, 1, -1)

        # Inpainting mask: at masked positions, replace input with mask_embed.
        # (For pure unconditional FM, mask=None.)
        if mask is not None:
        ...
        v_pos = per_joint[..., :self.cfg.C]               # velocity field for pos
        v_rot = per_joint[..., self.cfg.C:]               # velocity field for rot6d

        # Root velocity: pool MSA over joint axis, project to 3
        msa_pool = msa.mean(dim=2)                        # [B, T, H]
        v_root   = self.root_proj(msa_pool)               # [B, T, 3]
