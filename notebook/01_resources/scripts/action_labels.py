...
def family_distribution(subset_or_labels) -> dict:
    """Count clips per action family. Accepts:
      - torch Subset of MixedMotionDataset (reads .dataset.action_label[indices])
      - 1-D iterable of label strings.
    Returns a dict {family: count} including zero-count families.
    """
    from collections import Counter

    if hasattr(subset_or_labels, "dataset") and hasattr(subset_or_labels, "indices"):
        labels = subset_or_labels.dataset.action_label[subset_or_labels.indices]
    elif hasattr(subset_or_labels, "action_label"):
        labels = subset_or_labels.action_label
    else:
        labels = subset_or_labels

    counts = Counter(str(l) for l in labels)
    return {fam: counts.get(fam, 0) for fam in (*ALL_LABELS, *(set(counts) - set(ALL_LABELS)))}
